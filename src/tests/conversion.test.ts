  ...
  describe('convertToImperial', () => {
    describe('Solid ingredients (ounces)', () => {
      it('should convert grams to ounces for small amounts', () => {
        ...
        expect(convertToImperial(11)).toBe('0.39 oz'); // Kalamata olives test case
      });

      it('should convert larger amounts correctly', () => {
        expect(convertToImperial(1000)).toBe('35.27 oz');
        expect(convertToImperial(200)).toBe('7.05 oz'); // Chicken breast test case
      });

      it('should handle very small measurements', () => {
        expect(convertToImperial(0.6)).toBe('0.02 oz'); // Chili flakes test case
        expect(convertToImperial(1)).toBe('0.04 oz'); // Fresh basil test case
        expect(convertToImperial(3)).toBe('0.11 oz'); // Ginger test case
      });
    });

    describe('Liquid ingredients (fluid ounces)', () => {
      it('should convert millilitres to fluid ounces for small amounts', () => {
        expect(convertToImperial(5, true)).toBe('0.17 fl oz'); // Olive oil tsp
        expect(convertToImperial(15, true)).toBe('0.51 fl oz'); // Olive oil tbsp
        expect(convertToImperial(30, true)).toBe('1.01 fl oz'); // Sesame oil/soy sauce
      });

      it('should convert larger liquid amounts correctly', () => {
        expect(convertToImperial(100, true)).toBe('3.38 fl oz'); // Coconut milk
        expect(convertToImperial(1000, true)).toBe('33.81 fl oz'); // 1 litre
      });
