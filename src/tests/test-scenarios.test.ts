...
/**
 * Integration tests for the three main test scenarios described in the README:
...
describe('Recipe Test Scenarios Integration', () => {
  ...
  describe('Test Scenario Integration: All recipes working together', () => {
    it('should handle all three test scenarios with different characteristics', () => {
      ...
      // Verify we have all test scenarios
      expect(tunaSalad).toBeDefined();
      expect(stirFry).toBeDefined();
      expect(smoothie).toBeDefined();

      // Each should have different serving characteristics
      expect(tunaSalad.yield).toBe(1);    // Single serving
      expect(stirFry.yield).toBe(2);      // Multiple servings
      expect(smoothie.yield).toBe(1);     // Single serving

      // Each should have different ingredient counts
      expect(tunaSalad.ingredientsCollection.total).toBe(13); // Most complex
      expect(stirFry.ingredientsCollection.total).toBe(8);    // Medium complexity
      expect(smoothie.ingredientsCollection.total).toBe(6);   // Simplest
    });

    it('should provide comprehensive measurement range coverage', () => {
      const allIngredients = recipes.flatMap(recipe =>
        recipe.ingredientsCollection.items
          .filter(item => item.food && typeof item.metricMeasurement === 'number')
          .map(item => ({
            name: item.food!.name,
      ...
      const amounts = allIngredients.map(item => item.amount).filter(amount => amount > 0);
      
      // Should cover wide range of measurements
      const minAmount = Math.min(...amounts);
      const maxAmount = Math.max(...amounts);
      
      expect(minAmount).toBe(0.6);  // Chili flakes (very small)
      expect(maxAmount).toBe(200);  // Chicken breast (large)
      expect(maxAmount / minAmount).toBeGreaterThan(300); // Wide range coverage

      // Should have good distribution across measurement ranges
      const smallAmounts = amounts.filter(a => a <= 10);    // <= 10g/ml
      const mediumAmounts = amounts.filter(a => a > 10 && a <= 50);  // 10-50g/ml
      const largeAmounts = amounts.filter(a => a > 50);     // > 50g/ml

      expect(smallAmounts.length).toBeGreaterThan(5);   // Good small measurement coverage
      expect(mediumAmounts.length).toBeGreaterThan(3);  // Good medium measurement coverage  
      expect(largeAmounts.length).toBeGreaterThan(3);   // Good large measurement coverage
    });

    it('should test unit conversion accuracy across all scenarios', () => {
      // Test representative measurements from each scenario
      const testCases = [
        // Tuna Pasta Salad - variety of types and measurements
        { amount: 0.6, isLiquid: false, metric: '0.6 g', imperial: '0.02 oz', source: 'Chili flakes' },
        { amount: 5, isLiquid: true, metric: '5 ml', imperial: '0.17 fl oz', source: 'Tsp olive oil' },
        { amount: 50, isLiquid: false, metric: '50 g', imperial: '1.76 oz', source: 'Pasta' },
        { amount: 100, isLiquid: false, metric: '100 g', imperial: '3.53 oz', source: 'Tuna' },
        
        // Chicken Stir Fry - multiple servings, different scales
        { amount: 200, isLiquid: false, metric: '200 g', imperial: '7.05 oz', source: 'Chicken' },
        { amount: 150, isLiquid: false, metric: '150 g', imperial: '5.29 oz', source: 'Mixed vegetables' },
        { amount: 30, isLiquid: true, metric: '30 ml', imperial: '1.01 fl oz', source: 'Sesame oil' },
        
        // Smoothie Bowl - liquid-heavy, smaller measurements
        { amount: 100, isLiquid: true, metric: '100 ml', imperial: '3.38 fl oz', source: 'Coconut milk' },
        { amount: 12, isLiquid: false, metric: '12 g', imperial: '0.42 oz', source: 'Chia seeds' },
        { amount: 10, isLiquid: false, metric: '10 g', imperial: '0.35 oz', source: 'Granola' },
      ];
    ...
    it('should handle serving size scaling across different recipe types', () => {
      ...
      scenarios.forEach(({ recipe, testServings, expectedNutrition }) => {
        testServings.forEach(servings => {
          ...
          const expected = expectedNutrition[servings as keyof typeof expectedNutrition];
      ...
      recipes.forEach(recipe => {
        recipe.ingredientsCollection.items.forEach(ingredient => {
          if (ingredient.food && ingredient.metricMeasurement > 0) {
            ...
            const isLiquid = name.includes('oil') || name.includes('vinegar') || 
                           name.includes('milk') || name.includes('sauce');
            
            if (isLiquid) {
              liquidCount++;
              // Verify liquid formatting
              expect(formatMetric(ingredient.metricMeasurement, true)).toContain('ml');
            } else {
              solidCount++;
              // Verify solid formatting
              expect(formatMetric(ingredient.metricMeasurement, false)).toContain('g');
            }
          }
        });
      });
    ...
    it('should handle edge cases with very small measurements across scenarios', () => {
      ...
      // Find edge cases
      const zeroMeasurements = allMeasurements.filter(item => item.amount === 0);
      const verySmallMeasurements = allMeasurements.filter(item => item.amount > 0 && item.amount < 1);
      const smallMeasurements = allMeasurements.filter(item => item.amount >= 1 && item.amount <= 10);
    ...
    it('should simulate complete nutrition display workflow for all test scenarios', () => {
      recipes.forEach(recipe => {
        ...
        const displayedNutrition = {
          ...
          carbohydrate: recipe.nutritionalSummary.carbohydrate * currentServingSize,
        ...
        const displayedIngredients = recipe.ingredientsCollection.items
          ...
          .map(ingredient => {
            const scaledAmount = (ingredient.metricMeasurement || 0) * (currentServingSize / recipe.yield);
            const isLiquid = ingredient.food!.name.toLowerCase().includes('oil') || 
