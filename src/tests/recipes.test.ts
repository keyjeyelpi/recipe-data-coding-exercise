...
describe('Recipe Data Test Scenarios', () => {
  ...
  describe('Tuna Pasta Salad - Single serving, variety of ingredient types', () => {
    ...
    it('should have mixed solid and liquid ingredients', () => {
      ...
      const hasLiquids = ingredients.some(item => 
        item.food && liquidIngredients.includes(item.food.name)
      );
      const hasSolids = ingredients.some(item => 
        item.food && solidIngredients.includes(item.food.name)
      );
    ...
    it('should have ingredients with and without nutrition inclusion', () => {
      const ingredients = tunaSalad!.ingredientsCollection.items;
      const nutritionIncluded = ingredients.filter(item => item.includedInNutrition === true);
      const nutritionExcluded = ingredients.filter(item => item.includedInNutrition === false);
    ...
    it('should handle ingredients with zero measurements (to taste)', () => {
      const ingredients = tunaSalad!.ingredientsCollection.items;
      const zeroMeasurements = ingredients.filter(item => 
        item.food && item.metricMeasurement === 0
      );
      
      expect(zeroMeasurements.length).toBe(2); // Salt and pepper
      expect(zeroMeasurements.every(item => item.notes === 'to taste')).toBe(true);
    });
  });
  ...
  describe('Chicken Stir Fry - Multiple servings (2), different measurement scales', () => {
    ...
    it('should have asian cuisine specific ingredients', () => {
      const ingredients = stirFry!.ingredientsCollection.items;
      const asianIngredients = ['sesame oil', 'soy sauce', 'fresh ginger', 'sesame seeds'];
  ...
  describe('Smoothie Bowl - Single serving, liquid-heavy, smaller measurements', () => {
    ...
    it('should be liquid-heavy with precise small measurements', () => {
      ...
      const liquidIngredient = ingredients.find(item => 
        item.food && item.food.name === 'coconut milk'
      );
    ...
    it('should test edge cases with very small measurements', () => {
      ...
      const smallMeasurements = ingredients
        .filter(item => item.food && item.metricMeasurement > 0 && item.metricMeasurement <= 16)
        .map(item => ({ name: item.food!.name, amount: item.metricMeasurement }));
  ...
  describe('Cross-recipe comparison and edge cases', () => {
    it('should have different nutrition profiles per serving', () => {
      ...
      expect(Math.max(...proteinValues) - Math.min(...proteinValues)).toBeGreaterThan(10);
    });
    ...
    it('should have ingredients with various inclusion statuses', () => {
      ...
      const includedCount = allIngredients.filter(item => item.includedInNutrition === true).length;
      const excludedCount = allIngredients.filter(item => item.includedInNutrition === false).length;
